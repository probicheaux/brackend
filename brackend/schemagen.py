...
# Generates the directions for the losers of winners matches for entrant numbers where same_next == 0.
def losers_same_next_zero(winners, bracket):
    for i in range(len(winners) - 1):
        for j in range(len(winners[i])):
            if bracket["winners"][i]["seeds"][j]["teams"] != [
                {"name": "", "game": -1},
                {"name": "", "game": -1},
            ]:
                if i == 0:
                    ...
                    winners[i][j][1][3] = 1
                    cur = winners[i][j][1]
                    bracket[cur[0]][cur[1]]["seeds"][cur[2]]["teams"][cur[3]][
                        "name"
                    ] = [i, j]
                elif i == 1:
                    # print(len(bracket[0][0]), (len(bracket[0][0])//2-j-1)*2+1)
                    if bracket["winners"][0]["seeds"][
                        (len(bracket["winners"][0]["seeds"]) // 2 - j - 1) * 2 + 1
                    ]["teams"] != [{"name": "", "game": -1}, {"name": "", "game": -1}]:
                        # print("stuff")
                        winners[i][j][1][1] = 0
                        winners[i][j][1][2] = (
                            len(bracket["winners"][0]["seeds"]) // 2 - j - 1
                        )
                        winners[i][j][1][3] = 0
                        cur = winners[i][j][1]
                        bracket[cur[0]][cur[1]]["seeds"][cur[2]]["teams"][cur[3]][
                        ...
                        if base_pos % 2 == 1 and bracket["losers"][0]["seeds"][
                            base_pos - 1
                        ]["teams"] != [
                            {"name": "", "game": -1},
                            {"name": "", "game": -1},
        ...
        for j in range(len(winners[i])):
            if bracket["winners"][i]["seeds"][j]["teams"] != [
                ...
                if i == 0:
                    ...
                    if (
                        j % 2 == 1
                        and bracket["winners"][i]["seeds"][j - 1]["teams"]
                        != [{"name": "", "game": -1}, {"name": "", "game": -1}]
                    ) or (
                        j % 2 == 0
                        and bracket["winners"][i]["seeds"][j + 1]["teams"]
                        != [{"name": "", "game": -1}, {"name": "", "game": -1}]
...
# A schema is a set of instructions for where to send the winners and losers of each match in the
# bracket.
def makeSchemaJSON(bracket2, e):
    bracket = copy.deepcopy(bracket2)
    winners = [
        [
            [["winners", j + 1, None, None], ["losers", None, None, None]]
            for k in range(len(bracket["winners"][j]["seeds"]))
        ]
        for j in range(len(bracket["winners"]))
    ]
    losers = [
        [
            [["losers", j + 1, None, None], "OUT"]
            for k in range(len(bracket["losers"][j]["seeds"]))
        ]
        for j in range(len(bracket["losers"]))
    ]
    ...
    if same_next == 0:
        winners = losers_same_next_zero(winners, bracket)
    else:
